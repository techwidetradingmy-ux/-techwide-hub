...
function FullProfilePage({user,currentUserId,onBack,onDM}){
  ...
  const rows=[["💼 Position",user.role||user.position],["🎂 Age",age?`${age} years old`:null],["⚧ Gender",user.gender||null],["🏠 Home Town",user.hometown||null],["📱 Contact",user.contact_number?formatContact(user.contact_number):null],["📧 Email",user.email||null],["📅 Joining Date",user.joined_date_verified?fmtDate(user.joined_date):null],["⏳ Been Working",daysWorking!==null?`${daysWorking.toLocaleString()} days`:null],["🎂 Birthday",user.birthday_verified?fmtDate(user.birthday):null],["🎮 Hobby",user.hobby||null],["🍜 Fav Food",user.favorite_food||null]].filter(([,v])=>v);
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF}}>
      {showAvatarFull&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowAvatarFull(false)}><div style={{width:280,height:280,borderRadius:"50%",background:user.avatar_url?`url(${user.avatar_url}) center/cover`:`linear-gradient(145deg,${ORG},#ffb940)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:80,fontWeight:700,color:ACC,overflow:"hidden"}}>{!user.avatar_url&&(user.avatar||"?")}</div><button style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,.2)",border:"none",borderRadius:"50%",width:40,height:40,color:"#fff",fontSize:20,cursor:"pointer"}}>✕</button></div>)}
      {showBannerFull&&user.banner_url&&(<div style={{position:"fixed",inset:0,background:"#000",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowBannerFull(false)}><img src={user.banner_url} alt="banner" style={{width:"100%",maxHeight:"100vh",objectFit:"contain"}}/><button style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,.2)",border:"none",borderRadius:"50%",width:40,height:40,color:"#fff",fontSize:20,cursor:"pointer"}}>✕</button></div>)}
      ...
      <div style={{paddingBottom:50}}>
        ...
        <div style={{padding:"0 16px",marginTop:-54,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
          <div onClick={()=>setShowAvatarFull(true)} style={{width:96,height:96,borderRadius:"50%",border:`3px solid ${BG}`,background:user.avatar_url?`url(${user.avatar_url}) center/cover`:`linear-gradient(145deg,${ORG},#ffb940)`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:30,color:ACC,overflow:"hidden",flexShrink:0,cursor:"zoom-in"}}>{!user.avatar_url&&(user.avatar||"?")}</div>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:tier.color+"18",borderRadius:99,padding:"6px 14px",marginBottom:4}}><span style={{fontSize:16}}>{tier.emoji}</span><span style={{fontSize:14,color:tier.color,fontWeight:700}}>{tier.name}</span></div>
        </div>
        <div style={{padding:"0 16px 24px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:16,marginBottom:16}}>
            {[{l:"Points",v:(user.xp||0).toLocaleString(),e:"⚡"},{l:"Streak",v:`${user.streak||0}d`,e:"🔥"},{l:"Level",v:getLevel(user.xp||0),e:"⭐"}].map((s,i)=>(
              <div key={i} style={{background:BG2,borderRadius:14,padding:"14px 8px",textAlign:"center"}}><div style={{fontSize:22,marginBottom:4}}>{s.e}</div><div style={{fontSize:18,fontWeight:700,color:ACC}}>{s.v}</div><div style={{fontSize:12,color:LB3,marginTop:3}}>{s.l}</div></div>
            ))}
          </div>
          {rows.length>0&&(
...
export default function UserApp({profile:init,session,onProfileUpdate}){
  ...
  useEffect(()=>{
    ...
    const ch=supabase.channel("user_rt_v3_"+profile.id)
      // Instant points update when admin gifts or approves mission
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"profiles",filter:`id=eq.${profile.id}`},payload=>{
        if(payload.new)syncProfile({...profileRef.current,...payload.new});
      })
      ...
      // Notification INSERT — instant popup + sound
      ...
      <div style={{padding:"0 16px 16px"}}>
        ...
        <div className="fade" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
          {[{l:"Points",v:profile.xp.toLocaleString(),e:"⚡"},{l:"Streak",v:`${profile.streak}d`,e:"🔥"},{l:"Missions",v:completedCount,e:"✅"}].map((s,i)=>(
            <div key={i} style={{background:BG2,borderRadius:16,padding:"16px 10px",textAlign:"center"}}><div style={{fontSize:24,marginBottom:4}}>{s.e}</div><div style={{fontSize:20,fontWeight:700,color:ACC,letterSpacing:"-.5px",lineHeight:1}}>{s.v}</div><div style={{fontSize:12,color:LB3,marginTop:4}}>{s.l}</div></div>
          ))}
        </div>
  ...
  // ── TAB BAR HEIGHT for chat positioning ──
  // header ~58px, tabbar ~82px + safe area
  const HEADER_H=58;
  const TABBAR_H=82;
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF,maxWidth:430,margin:"0 auto",display:"flex",flexDirection:"column"}}>
      ...
      {/* Regular content area (non-chat tabs) */}
      {tab!=="chat"&&(
        <div key={tabKey} className="page-enter-forward" style={{flex:1,overflowY:"auto",padding:"14px 0 120px"}}>
          {tab==="home"    &&<HomeTab/>}
      ...
      {/* CHAT TAB — fixed between header and tab bar, input always visible */}
      {tab==="chat"&&(
      ...
      {toast&&(
        <div className="toast-in" style={{position:"fixed",bottom:110,left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,.82)",backdropFilter:"blur(16px)",borderRadius:99,padding:"12px 24px",fontSize:15,color:"#fff",fontWeight:600,whiteSpace:"nowrap",zIndex:50,pointerEvents:"none",maxWidth:"88vw",textAlign:"center"}}>
          {toast}
        </div>
      )}
    </div>
