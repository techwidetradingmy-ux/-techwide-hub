...
function OnboardingFlow({user,onComplete}){
  ...
  const [form,setForm]=useState({
    name:user.name||"",nickname:"",birthday:"",joined_date:new Date().toISOString().split("T")[0],
  ...
  const inp=(k,ph,type="text",required=false)=>(
    <div>
      <input
        value={form[k]}
        onChange={e=>{
          if(k==="ic_number"){set("ic_number",formatIC(e.target.value));clearErr("ic_number");}
          else if(k==="bank_account"){set("bank_account",e.target.value.replace(/\D/g,''));clearErr("bank_account");}
          else if(k==="contact_number"){set("contact_number",e.target.value.replace(/\D/g,''));clearErr("contact_number");}
          else{set(k,e.target.value);clearErr(k);}
        }}
        placeholder={ph}
        type={type}
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF,maxWidth:430,margin:"0 auto"}}>
      {showCrop&&rawImg&&<CircleCrop src={rawImg} onCrop={handleCropDone} onCancel={()=>{setShowCrop(false);setRawImg(null);}}/>}
      ...
      <div style={{padding:"0 16px 24px"}}>
        {step===1&&(
          <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
            {FR("Full Name",inp("name","Ahmad Farid",  "text"),false,true)}
            {FR("Nickname",inp("nickname","Farid"),false,false)}
            {FR("Birthday",inp("birthday","","date"),false,true)}
            {FR("Joining Date",inp("joined_date","","date"),true,false)}
          </div>
        )}

        {step===2&&(
          <>
            <div style={{background:`${ACC}10`,borderRadius:12,padding:"12px 14px",marginBottom:12,fontSize:13,color:ACC,lineHeight:1.6}}>
              📱 Enter numbers only (no spaces or dashes)<br/>
              Format: starts with 01X (e.g. 0123456789)<br/>
              For 011 numbers: 01XXXXXXXXX (11 digits)
            </div>
            <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
              {FR("Contact Number",
                <div>
        ...
        {step===3&&(
          <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
            ...
            {FR("Hobby",inp("hobby","e.g. Gaming, Reading"))}
            {FR("Favourite Food",inp("favorite_food","e.g. Nasi Lemak"),true)}
          </div>
        )}
        ...
        {step===4&&(
          <>
            <div style={{background:`${ACC}10`,borderRadius:12,padding:"12px 14px",marginBottom:12,fontSize:13,color:ACC,lineHeight:1.7}}>
              🔒 This info is only visible to admin.<br/>
              IC, EPF & Bank will require admin verification before being shown as verified.
            </div>
            <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
              {FR("IC Number",
                <div>
              ...
              ,false,true)}
              {FR("EPF Number",inp("epf_number","XXXXXXXXXXXX"))}
              ...
              <div style={{padding:"11px 16px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {BANK_TYPES.map(b=>(
                    <button key={b} onClick={()=>set("bank_type",b)}
                      style={{padding:"8px 10px",background:form.bank_type===b?ACC:"rgba(0,0,0,.05)",color:form.bank_type===b?"#fff":LBL,border:"none",borderRadius:8,fontSize:13,cursor:"pointer",fontFamily:SF,textAlign:"left",fontWeight:form.bank_type===b?600:400}}>
                      {b}
                    </button>
                  ))}
                </div>
        ...
        {step===5&&(
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            {form.avatar_url&&(
              <button onClick={()=>document.getElementById("avOnboard").click()} style={{fontSize:15,color:ACC,fontWeight:600,background:"none",border:"none",cursor:"pointer",fontFamily:SF}}>Change Photo</button>
            )}
            <div style={{fontSize:13,color:LB3,marginTop:12,lineHeight:1.6}}>Your photo will be cropped to a circle.<br/>You can update this anytime in your profile.</div>
          </div>
        )}
