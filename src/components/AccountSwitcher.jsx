 ...
 *   onSwitch    – (account) => void — called when user picks a different account
 *   onAddNew    – () => void — called when user taps "Add Account"
 */
export default function AccountSwitcher({ show, onClose, onSwitch, onAddNew }) {
  ...
  const handleSwitch = async (acct) => {
    ...
    try {
      const target = await switchToAccount(acct.id);
      onSwitch(target);
    } catch (err) {
            ...
            <div key={acct.id} style={row(isActive)} onClick={() => handleSwitch(acct)}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  ...
                  {isActive && (
                    <span style={{
                      fontSize: 11, color: ACC, fontWeight: 700,
                ...
                <div style={{ fontSize: 13, color: LB3, marginTop: 1 }}>
                  {acct.email}
                  {acct.is_admin && " · Admin"}
                </div>
              </div>
