...
// ── BRANDED DM CHAT — fixed layout so input never hides ──────────────
function DMChat({profile,dmWith,allProfiles,onBack,setViewingProfile}){
  ...
  return(
    // ── KEY FIX: use position:fixed + flex column so input never hides ──
    <div
      style={{
        position:"fixed",
        top:0,left:"50%",
      ...
      <div style={{background:ACC,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,flexShrink:0,paddingTop:"max(12px,env(safe-area-inset-top))"}}>
        <button onClick={onBack} className="btn"
          style={{background:"none",border:"none",color:"#fff",fontSize:22,cursor:"pointer",padding:"4px 8px 4px 0",fontFamily:SF}}>←</button>
      ...
      {/* Messages — flex:1 + minHeight:0 is CRITICAL for proper scroll */}
      ...
      {/* Emoji picker — above input, flexShrink:0 */}
      {showEmoji&&<EmojiPicker onSelect={e=>{setText(p=>p+e);inputRef.current?.focus();}}/>}

      {/* Attach menu — above input, flexShrink:0 */}
      {showAttach&&(
      ...
      {/* ── INPUT BAR — always at bottom, flexShrink:0 ── */}
      <div style={{
        background:BG2,
        padding:"10px 12px",
        display:"flex",
...
// ── MISSION DETAIL PAGE — no sticky header ───────────────────────────
function MissionDetailPage({mission,claim,profile,onBack,onAccept,onDecline,onRefresh,showToast,Chip}){
  ...
  // ── FIX: check both completed flag AND status ──
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF}}>
      {/* ── Floating back button only — no full header bar ── */}
      ...
      <div style={{padding:"12px 16px 50px"}}>
        {/* Hero card — NO "Campaign Mission" header label */}
        <div style={{background:`linear-gradient(135deg,${ACC},#0e2140)`,borderRadius:18,padding:22,marginBottom:18,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}>
            ...
            {isDeclined&&<Chip color="#8e8e93">🚫 Not Interested</Chip>}
        ...
        {done?(
          ...
          <div style={{background:`linear-gradient(135deg,${ACC}10,${ORG}08)`,border:`2px solid ${ACC}20`,borderRadius:20,padding:"32px 24px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
            ...
            <div style={{fontSize:15,color:LB2,lineHeight:1.65,maxWidth:280,textAlign:"center"}}>Your proof is under admin review. You'll be notified once it's approved.</div>
...
export function MissionsTab({profile,missions,myClaims,setMyClaims,showToast,Chip,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG}){
  ...
  // Helper: get claim for a mission
  const claimOf=id=>myClaims.find(c=>c.mission_id===id);

  // ── FIX: categorize properly checking completed flag ──
  ...
  const COLUMNS=[
    ...
    {id:"completed",label:"Completed",  emoji:"✅", list:completedMissions,count:completedMissions.length},
    {id:"skipped",  label:"Skipped",    emoji:"🚫", list:skippedMissions,  count:skippedMissions.length},
  ];

  // ── Accept: insert/upsert claim with status='accepted' ──
  const accept=async missionId=>{
    const existing=claimOf(missionId);
    if(existing){
      // Reactivate from declined
  ...
  // ── Decline: upsert with status='declined' (don't delete) ──
  ...
  return(
    <div style={{padding:"0 16px 16px"}}>
      ...
      <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8,marginBottom:14,scrollbarWidth:"none"}}>
        {COLUMNS.map(col=>(
          <button key={col.id} onClick={()=>setColumn(col.id)} className="btn"
            ...
            {col.label}
            {col.count>0&&(
              <span style={{background:column===col.id?"rgba(255,255,255,.3)":"rgba(0,0,0,.12)",color:column===col.id?"#fff":LBL,fontSize:12,fontWeight:700,padding:"2px 8px",borderRadius:99,lineHeight:1.5}}>
                {col.count}
              </span>
            )}
          </button>
      ...
      <div style={{background:colColor+"10",borderRadius:14,padding:"12px 16px",marginBottom:14,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          <div style={{fontSize:16,fontWeight:700,color:colColor}}>{currentCol?.label} Missions</div>
          <div style={{fontSize:13,color:LB3,marginTop:2}}>
            {column==="new"&&"New missions available for you to accept"}
            {column==="ongoing"&&"Missions you've accepted — submit proof when done"}
            {column==="completed"&&"Missions you've successfully completed 🎉"}
            {column==="skipped"&&"Missions you skipped — you can still accept them"}
          </div>
        </div>
      </div>
      ...
      {list.map(m=>{
        ...
        return(
          <div key={m.id} onClick={()=>setSelected(m)} className="card-press"
            style={{background:BG2,borderRadius:16,padding:"16px",marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:14,opacity:done?1:overdue&&column==="new"?.6:1}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                <div style={{fontSize:17,color:done?LB3:LBL,fontWeight:600,letterSpacing:"-.3px",lineHeight:1.3,textDecoration:done?"line-through":"none",flex:1}}>
                  {m.title}
                </div>
...
export function CommunityTab({profile,allProfiles,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG,dmTarget,setDmTarget,setViewingProfile}){
  ...
  return(
    // ── height:100% fills the flex container in UserApp ──
    <div style={{height:"100%",display:"flex",flexDirection:"column",overflow:"hidden",background:BG}}>
      ...
      {/* Input bar — always visible, flexShrink:0 */}
...
export function ProfileTab({profile,syncProfile,score,tier,completedCount,showToast,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG}){
  ...
  const submitPrivate=async(fieldName,value,extra=null)=>{
    ...
    await loadVerif();showToast("Submitted for admin verification 🔒");setPrivateEditing(null);setSubmittingPrivate(false);
  };
    ...
    <div style={{paddingBottom:16}}>
      ...
      <div style={{padding:"0 16px",marginTop:-48,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        <div style={{position:"relative"}}>
          <div onClick={()=>!editing&&setShowAvatarFull(true)} style={{width:90,height:90,borderRadius:"50%",background:av?`url(${av}) center/cover`:`linear-gradient(145deg,${ORG},#ffb940)`,border:`3px solid ${BG}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:700,color:ACC,overflow:"hidden",cursor:!editing?"zoom-in":"default"}}>{!av&&(profile.avatar||"?")}</div>
        ...
        editing?(
          <div style={{padding:"0 16px"}}>
            <div style={{background:BG2,borderRadius:16,overflow:"hidden",marginBottom:12}}>
              {[{label:"Full Name",key:"name",ph:"Ahmad Farid"},{label:"Nickname",key:"nickname",ph:"e.g. Farid"},{label:"Hobby",key:"hobby",ph:"e.g. Gaming"},{label:"Favourite Food",key:"favorite_food",ph:"e.g. Nasi Lemak"},{label:"Home Town",key:"hometown",ph:"e.g. Kuala Lumpur"},{label:"Email",key:"email",ph:"your@email.com",type:"email"}].map(({label,key,ph,type="text"},i,a)=>(
                <div key={key} style={{padding:"13px 16px",borderBottom:i<a.length-1?`1px solid ${SEP}`:"none"}}>
                  ...
                  <input value={form[key]} onChange={e=>set(key,e.target.value)} placeholder={ph} type={type} style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:18,color:LBL,fontFamily:SF}}/>
                </div>
              ))}
              <div style={{padding:"13px 16px",borderBottom:`1px solid ${SEP}`}}>
                ...
                <input value={form.contact_number} onChange={e=>set("contact_number",e.target.value.replace(/\D/g,""))} placeholder="0123456789" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:18,color:LBL,fontFamily:SF}}/>
              ...
              <div style={{padding:"13px 16px",borderBottom:`1px solid ${SEP}`}}>
                ...
                <textarea value={form.bio} onChange={e=>set("bio",e.target.value)} placeholder="Short description…" rows={3} style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:18,color:LBL,resize:"none",fontFamily:SF,lineHeight:1.5}}/>
              </div>
              <div style={{padding:"13px 16px",borderBottom:`1px solid ${SEP}`}}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:20}}>
              <button onClick={()=>setEditing(false)} className="btn" style={{flex:1,padding:"16px",background:"rgba(0,0,0,.05)",border:"none",borderRadius:14,fontSize:17,color:"#ff3b30",cursor:"pointer",fontFamily:SF,fontWeight:600}}>Cancel</button>
        ...
        <div style={{padding:"0 16px"}}>
          ...
          <div style={{background:BG2,borderRadius:16,overflow:"hidden",marginBottom:10}}>
            <div style={{padding:"14px 16px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:14,color:LB3,letterSpacing:".4px",textTransform:"uppercase",fontWeight:700}}>IC Number</div><VerifBadge field="ic_number"/></div>
              ...
              {!profile.ic_verified&&(privateEditing==="ic"?(
                <div style={{borderTop:`1px solid ${SEP}`,paddingTop:14}}>
                  <input value={privateForm.ic_number} onChange={e=>setPrivateForm(p=>({...p,ic_number:formatIC(e.target.value)}))} placeholder="XXXXXX-XX-XXXX" style={{width:"100%",background:"#f2f2f7",border:`1px solid ${SEP}`,borderRadius:12,padding:"12px 14px",fontSize:17,color:LBL,outline:"none",marginBottom:8,letterSpacing:1}}/>
          ...
          <div style={{background:BG2,borderRadius:16,overflow:"hidden",marginBottom:10}}>
            <div style={{padding:"14px 16px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:14,color:LB3,letterSpacing:".4px",textTransform:"uppercase",fontWeight:700}}>EPF Number</div><VerifBadge field="epf_number"/></div>
              ...
              {!profile.epf_verified&&(privateEditing==="epf"?(
                <div style={{borderTop:`1px solid ${SEP}`,paddingTop:14}}>
                  <input value={privateForm.epf_number} onChange={e=>setPrivateForm(p=>({...p,epf_number:e.target.value.replace(/\D/g,"")}))} placeholder="XXXXXXXXXXXX" type="tel" style={{width:"100%",background:"#f2f2f7",border:`1px solid ${SEP}`,borderRadius:12,padding:"12px 14px",fontSize:17,color:LBL,outline:"none",marginBottom:10}}/>
                  <div style={{display:"flex",gap:10}}>
                    <button onClick={()=>setPrivateEditing(null)} className="btn" style={{flex:1,padding:"12px",background:"rgba(0,0,0,.05)",border:"none",borderRadius:12,fontSize:15,color:LB2,cursor:"pointer",fontFamily:SF}}>Cancel</button>
          ...
          <div style={{background:BG2,borderRadius:16,overflow:"hidden",marginBottom:10}}>
            <div style={{padding:"14px 16px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontSize:14,color:LB3,letterSpacing:".4px",textTransform:"uppercase",fontWeight:700}}>Bank Account</div><VerifBadge field="bank_account"/></div>
              ...
              {privateEditing==="bank"?(
                <div style={{borderTop:`1px solid ${SEP}`,paddingTop:14}}>
                  <input value={privateForm.bank_account} onChange={e=>setPrivateForm(p=>({...p,bank_account:e.target.value.replace(/\D/g,"")}))} placeholder="Numbers only" type="tel" style={{width:"100%",background:"#f2f2f7",border:`1px solid ${SEP}`,borderRadius:12,padding:"12px 14px",fontSize:17,color:LBL,outline:"none",marginBottom:10}}/>
                  <div style={{position:"relative",marginBottom:10}}>
                    <select value={privateForm.bank_type} onChange={e=>setPrivateForm(p=>({...p,bank_type:e.target.value}))} style={{width:"100%",background:"transparent",border:`1px solid ${SEP}`,borderRadius:12,padding:"12px 36px 12px 14px",fontSize:17,color:LBL,appearance:"none",WebkitAppearance:"none",cursor:"pointer",outline:"none"}}>
