...
function DMChat({profile,dmWith,allProfiles,onBack,setViewingProfile}){
  ...
  const handleFile=async(e,type="image")=>{
    ...
    reader.onload=async ev=>{
      ...
      await send(label,type,ev.target.result,file.name);
    };
  ...
  return(
    <div
      style={{position:"fixed",inset:0,background:BG,zIndex:150,display:"flex",flexDirection:"column",maxWidth:430,margin:"0 auto",fontFamily:SF}}
      ...
      <div style={{background:ACC,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,flexShrink:0,paddingTop:"max(12px,env(safe-area-inset-top))"}}>
        <button onClick={onBack} className="btn"
          style={{background:"none",border:"none",color:"#fff",fontSize:22,cursor:"pointer",padding:"4px 8px 4px 0",fontFamily:SF,display:"flex",alignItems:"center"}}>
          ←
        </button>
      ...
      {/* Attach menu */}
      {showAttach&&(
        <div style={{background:BG2,padding:"16px",display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap",borderTop:`1px solid ${SEP}`,flexShrink:0}}>
          {[
      ...
      {/* Input bar */}
      <div style={{background:BG2,padding:"8px 12px",display:"flex",alignItems:"center",gap:8,flexShrink:0,borderTop:`1px solid ${SEP}`,paddingBottom:"max(8px,env(safe-area-inset-bottom))"}}>
...
function MissionDetailPage({mission,claim,profile,onBack,onAccept,onDecline,onRefresh,showToast,Chip}){
  ...
  const isSubmitted=submitted||localClaim?.submitted;
      ...
      <div style={{padding:"18px 16px 50px"}}>
        ...
        <div style={{background:`linear-gradient(135deg,${ACC},#0e2140)`,borderRadius:18,padding:22,marginBottom:18,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}>
            ...
            {isSubmitted&&!done&&<Chip color="#ff9500">⏳ Submitted</Chip>}
            {accepted&&!isSubmitted&&!done&&<Chip color={ACC}>✓ Accepted</Chip>}
            {dl!==null&&(
        ...
        {/* Action area */}
        {done?(
          <div style={{background:"#34c75910",borderRadius:16,padding:"28px 20px",textAlign:"center"}}>
            <div style={{fontSize:44,marginBottom:10}}>🎉</div>
            <div style={{fontSize:20,color:"#34c759",fontWeight:700,marginBottom:6}}>Mission Completed!</div>
            <div style={{fontSize:15,color:LB3}}>Reward has been added to your account</div>
          </div>
        ):isSubmitted?(
          /* Centred submission confirmation */
          <div style={{background:`linear-gradient(135deg,${ACC}10,${ORG}08)`,border:`2px solid ${ACC}20`,borderRadius:20,padding:"32px 24px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
            ...
            <div style={{fontSize:15,color:LB2,lineHeight:1.65,maxWidth:280,textAlign:"center"}}>
              Your proof has been sent to the admin for review. You'll receive a notification once it's approved.
            </div>
            <div style={{background:BG2,borderRadius:12,padding:"10px 20px",fontSize:14,color:LB3,fontWeight:500}}>
              ⏱️ Usually reviewed within 24 hours
            </div>
          </div>
        ):showSubForm?(
          <div style={{background:BG2,borderRadius:16,padding:"18px"}}>
            ...
            <textarea value={subText} onChange={e=>setSubText(e.target.value)}
              placeholder="Describe what you did, achievements, results…" rows={4}
    ...
    <div style={{padding:"0 16px 16px"}}>
      ...
      {list.length===0&&(
        <div style={{textAlign:"center",padding:"50px 0",color:LB3,fontSize:17}}>
          <div style={{fontSize:48,marginBottom:14}}>🎯</div>No missions available
        </div>
      )}
              ...
              return(
                <div key={p.id||p.name} className="fade card-press"
                  ...
                  <button
                    ...
                    style={{width:"100%",padding:"12px",background:(!can||out)?"rgba(0,0,0,.06)":ACC,color:(!can||out)?LB3:"#fff",border:"none",borderRadius:12,fontSize:15,fontWeight:700,cursor:(!can||out)?"default":"pointer",fontFamily:SF,transition:"background .15s"}}>
...
export function CommunityTab({profile,allProfiles,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG,dmTarget,setDmTarget,setViewingProfile}){
  ...
  const loadGroupMsgs=async()=>{
    const{data}=await supabase.from("messages")
      .select("*, sender:user_id(avatar_url,avatar)")
      .eq("is_dm",false).order("created_at",{ascending:true}).limit(100);
    if(data)setMessages(data);
  };

  // DM — full screen overlay, no outer tabs/headers visible
  if(mode==="dm"&&dmWith){
    return(
      <div style={{position:"fixed",inset:0,zIndex:150,maxWidth:430,margin:"0 auto"}}>
        <DMChat
  ...
  return(
    // height:100% fills the fixed container set in UserApp for chat tab
    <div style={{height:"100%",display:"flex",flexDirection:"column",overflow:"hidden",background:BG}}>
      ...
      {/* Messages — flex:1 + minHeight:0 keeps it scrollable */}
      ...
      {/* Input — always visible, pinned to bottom */}
    ...
    <div style={{paddingBottom:16}}>
      ...
      {showBannerFull&&(
        <div style={{position:"fixed",inset:0,background:"#000",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowBannerFull(false)}>
          {bn?<img src={bn} alt="banner" style={{width:"100%",maxHeight:"100vh",objectFit:"contain"}}/>:<div style={{color:"#fff",fontSize:16}}>No banner set</div>}
      ...
      {showAvatarFull&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowAvatarFull(false)}>
          <div style={{width:290,height:290,borderRadius:"50%",background:av?`url(${av}) center/cover`:`linear-gradient(145deg,${ORG},#ffb940)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:80,fontWeight:700,color:ACC,overflow:"hidden"}}>
            {!av&&(profile.avatar||"?")}
          </div>
          <button style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,.2)",border:"none",borderRadius:"50%",width:40,height:40,color:"#fff",fontSize:20,cursor:"pointer"}}>✕</button>
        </div>
      ...
      {section==="private"&&(
        <div style={{padding:"0 16px"}}>
          <div style={{background:`${ACC}10`,borderRadius:14,padding:"14px 16px",marginBottom:16,fontSize:15,color:ACC,lineHeight:1.7}}>
            ...
            <strong>Bank</strong> always editable (needs re-approval).
          </div>
          ...
          <div style={{background:BG2,borderRadius:16,overflow:"hidden",marginBottom:10}}>
            <div style={{padding:"14px 16px"}}>
              ...
              {privateEditing==="bank"?(
                ...
                <button onClick={()=>setPrivateEditing("bank")} className="btn"
                  style={{width:"100%",padding:"12px",background:`${ACC}10`,border:"none",borderRadius:12,fontSize:15,color:ACC,fontWeight:700,cursor:"pointer",fontFamily:SF}}>
