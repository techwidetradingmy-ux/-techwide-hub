...
const GLOBAL_CSS=`
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;}
  ...
  .react-datepicker{font-family:${SF}!important;border:none!important;border-radius:14px!important;box-shadow:0 8px 32px rgba(0,0,0,.18)!important;overflow:hidden}
  .react-datepicker__header{background:${ACC}!important;border-bottom:none!important;padding-top:12px!important;border-radius:0!important}
  .react-datepicker__current-month,.react-datepicker__day-name{color:#fff!important;font-family:${SF}!important}
  .react-datepicker__day--selected,.react-datepicker__day--keyboard-selected{background:${ACC}!important;border-radius:50%!important;color:#fff!important}
  .react-datepicker__day--today{font-weight:700;color:${ORG}!important}
  .react-datepicker__day:hover{border-radius:50%!important;background:${ACC}22!important}
  .react-datepicker__navigation-icon::before{border-color:#fff!important}
  .react-datepicker__month-select,.react-datepicker__year-select{background:${ACC};color:#fff;border:none;border-radius:6px;padding:3px 6px;font-size:13px;cursor:pointer}
...
const ensureProfile=async(user)=>{
  try{
    ...
    const{data:cr,error}=await supabase.from("profiles").upsert({
      id:user.id,name:(user.email||"").split("@")[0],email:user.email||"",
      avatar,xp:0,streak:0,badges:[],is_admin:false,onboarded:false,contribution_score:0,
...
function SignUpScreen({onBack,onSignedIn}){
  ...
  useEffect(()=>{
    ...
    if(!prefix.trim()||!validPrefix(prefix)){setEmailStatus(null);setChecking(false);return;}
    ...
    timerRef.current=setTimeout(async()=>{
      ...
      try{
        ...
        try{const{data}=await supabase.from("profiles").select("id").eq("email",email).maybeSingle();setEmailStatus(data?"taken":"available");}
  ...
  const btnLabel=()=>{
    if(loading)return"Creating Account…";if(checking)return"Checking email…";
    if(emailStatus==="taken")return"Email already taken ✕";if(emailStatus==="error")return"Check failed — retry";
    if(!prefix.trim())return"Enter your username";if(!passOk)return"Password too short";
    if(!matchOk&&confirm)return"Passwords don't match";if(canSubmit)return"Create Account";
  ...
  const handleSignUp=async()=>{
    ...
    try{
      ...
      if(!sd?.user){setFormErr({general:"Account creation failed."});setLoading(false);return;}
      await supabase.from("profiles").upsert({
        id:sd.user.id,name:prefix.trim(),email,avatar:prefix.trim().slice(0,2).toUpperCase(),
      ...
      if(prof)onSignedIn(sid.session,prof);else onBack(email);
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF,maxWidth:430,margin:"0 auto"}}>
      <div style={{padding:"52px 16px 24px"}}>
        <button onClick={()=>onBack()} className="btn" style={{display:"flex",alignItems:"center",gap:6,color:ACC,fontSize:16,fontWeight:600,background:"none",border:"none",marginBottom:24,padding:0}}>← Back to Sign In</button>
        ...
        <div style={{fontSize:15,color:LB3,lineHeight:1.6}}>Create your Techwide Hub account.<br/>Your email will be <strong>yourname{DOMAIN}</strong></div>
      </div>
      <div style={{padding:"0 16px 40px"}}>
        <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
          <div style={{padding:"11px 16px",borderBottom:`1px solid ${SEP}`}}>
            ...
            <div style={{display:"flex",alignItems:"center",borderRadius:10,overflow:"hidden",border:`1.5px solid ${emailBorder}`,transition:"border-color .2s"}}>
              <input value={prefix} onChange={e=>{setPrefix(e.target.value.toLowerCase().replace(/\s/g,""));setFormErr(p=>({...p,prefix:undefined}));}}
          ...
          <div style={{padding:"11px 16px",borderBottom:`1px solid ${SEP}`}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input value={pass} onChange={e=>{setPass(e.target.value);setFormErr(p=>({...p,pass:undefined}));}}
                placeholder="Minimum 6 characters" type={showPass?"text":"password"} autoComplete="new-password"
                style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL}}/>
              <button type="button" onClick={()=>setShowPass(v=>!v)} style={{background:"none",border:"none",cursor:"pointer",padding:0,color:LB3,display:"flex",alignItems:"center",flexShrink:0}}>
                {showPass
          ...
          <div style={{padding:"11px 16px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input value={confirm} onChange={e=>{setConfirm(e.target.value);setFormErr(p=>({...p,confirm:undefined}));}}
                placeholder="Re-enter your password" type={showConfirm?"text":"password"} autoComplete="new-password"
                onKeyDown={e=>e.key==="Enter"&&canSubmit&&handleSignUp()}
                style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL}}/>
              <button type="button" onClick={()=>setShowConfirm(v=>!v)} style={{background:"none",border:"none",cursor:"pointer",padding:0,color:LB3,display:"flex",alignItems:"center",flexShrink:0}}>
                {showConfirm
                  ?<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
                  :<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                }
        ...
        <div style={{textAlign:"center",fontSize:13,color:LB3}}>Already have an account?{" "}<button onClick={()=>onBack()} className="btn" style={{color:ACC,fontWeight:600,background:"none",border:"none",cursor:"pointer",fontSize:13}}>Sign In</button></div>
      </div>
    </div>
  );
}
...
// ── ONBOARDING ─────────────────────────────────────────────────────────
function OnboardingFlow({user,onComplete,onBack}){
  ...
  const handleSubmit=async()=>{
    ...
    try{
      ...
      const payload={...form,contact_number:contact,birthday:birthdayISO,joined_date:joinedISO,bank_type:finalBankType,position:finalPosition,role:finalPosition,onboarded:true};
      ...
      reqs.push({user_id:user.id,field_name:"birthday",field_value:birthdayISO,status:"Pending"});
  ...
  return(
    <div style={{minHeight:"100vh",background:BG,fontFamily:SF,maxWidth:430,margin:"0 auto"}}>
      ...
      <div style={{padding:"52px 16px 24px"}}>
        {step===1&&onBack&&(
          <button onClick={onBack} className="btn" style={{display:"flex",alignItems:"center",gap:6,color:ACC,fontSize:15,fontWeight:600,background:"none",border:"none",marginBottom:20,padding:0,cursor:"pointer"}}>← Back</button>
        )}
      ...
      <div style={{padding:"0 16px 24px"}}>
        {step===1&&(
          <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
            ...
            <div style={{padding:"11px 16px",borderBottom:`1px solid ${SEP}`}}>
              ...
              {DateInp("birthday")}<ErrMsg k="birthday"/>
            </div>
            <div style={{padding:"11px 16px"}}>
              <div style={{display:"flex",gap:4,alignItems:"center",marginBottom:5}}>
              ...
              {DateInp("joined_date")}<ErrMsg k="joined_date"/>
            </div>
          </div>
        )}
        {step===2&&(
          <>
            ...
            <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
              {FW("Contact Number",
                <>
                  <input value={form.contact_number} onChange={e=>{set("contact_number",e.target.value.replace(/\D/g,""));clearErr("contact_number");}}
                    placeholder="0123456789" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL}}/>
        ...
        {step===4&&(
          <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:8}}>
            ...
            {FW("EPF Number",
              <>
                <input value={form.epf_number} onChange={e=>{set("epf_number",e.target.value.replace(/\D/g,""));clearErr("epf_number");}}
                  placeholder="XXXXXXXXXXXX" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL}}/>
                <ErrMsg k="epf_number"/>
              </>,false,true)}
            {FW("Bank Account Number",
              <>
                <input value={form.bank_account} onChange={e=>{set("bank_account",e.target.value.replace(/\D/g,""));clearErr("bank_account");}}
                  placeholder="Numbers only" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL}}/>
            ...
            <div style={{padding:"11px 16px",borderBottom:form.bank_type==="Others"?`1px solid ${SEP}`:"none"}}>
              ...
              <div style={{position:"relative"}}>
                <select value={form.bank_type} onChange={e=>{set("bank_type",e.target.value);clearErr("bank_type_other");set("bank_type_other","");}}
        ...
        {step===5&&(
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            {form.avatar_url&&<button onClick={()=>document.getElementById("avOnboard").click()} className="btn" style={{fontSize:15,color:ACC,fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>Change Photo</button>}
...
export default function App(){
  ...
  if(!session||!profile){
    ...
    const handleSavedAccountTap=async(acct)=>{
      // Try seamless login with stored tokens
      const tokens=getAccountSession(acct.id);
      if(tokens?.access_token&&tokens?.refresh_token){
        ...
        try{
          ...
          const p=await ensureProfile(data.user);
          if(p){upsertAccount(data.session,p);setSession(data.session);setProfile(p);setLoginLoading(false);return;}
        }catch(e){console.warn("Token login failed:",e);}
        setLoginLoading(false);
      }
      // Fallback: prefill email
      setLoginEmail(acct.authEmail||acct.email);setLoginErr("");
    };
    return(
      <div style={{minHeight:"100vh",background:BG,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:SF}}>
        <div className="fade" style={{textAlign:"center",marginBottom:28}}>
                  ...
                  return(
                    <div key={acct.id} onClick={()=>handleSavedAccountTap(acct)}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:13,color:LB3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{acct.authEmail||acct.email}</div>
                      </div>
  ...
  if(!profile.onboarded)return(
    <OnboardingFlow
      user={profile}
      onBack={async()=>{await supabase.auth.signOut();setProfile(null);setSession(null);}}
      onComplete={async()=>{
  ...
  const handleAccountSwitch=async(accountId)=>{
    ...
    let tokens=null;
    try{tokens=switchToAccount(accountId);}catch(e){console.warn("No stored tokens:",e);}
    if(tokens?.access_token&&tokens?.refresh_token){
      setLoading(true);
      try{
        const{data,error}=await supabase.auth.setSession({access_token:tokens.access_token,refresh_token:tokens.refresh_token});
    ...
    if(acct)setLoginEmail(acct.authEmail||acct.email);
