...
function UserProfileModal({user, currentUserId, onClose, onDM, ACC, ORG, BG, BG2, SEP, LBL, LB2, LB3, SF, getTier, calcScore}){
  const tier=getTier(calcScore(user.joined_date,0));
  const age=user.birthday?Math.floor((Date.now()-new Date(user.birthday))/(365.25*24*60*60*1000)):null;
  return(
      ...
      <div style={{background:BG,borderRadius:"20px 20px 0 0",maxHeight:"90vh",overflowY:"auto",paddingBottom:40}}>
        ...
        <div style={{padding:"0 16px 16px"}}>
          ...
          <div style={{background:BG2,borderRadius:13,overflow:"hidden"}}>
            {[
              ["💼 Position",   user.role],
              ["📅 Joined",     user.joined_date?new Date(user.joined_date).toLocaleDateString("en-MY",{day:"numeric",month:"long",year:"numeric"}):null],
              ["🎂 Age",        age?`${age} years old`:null],
              ["📱 Contact",    user.contact_number?formatContact(user.contact_number):null],
              ["📧 Email",      user.email],
              ["🎮 Hobby",      user.hobby],
              ["🍜 Fav Food",   user.favorite_food],
    ...
    <div style={{padding:"0 16px 12px"}}>
      ...
      <Section className="fade">
        {allProfiles.map((s,i)=>{
          ...
          return(
            <div key={s.id} onClick={()=>setViewUser(s)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:me?`${ACC}08`:BG2,borderBottom:i<allProfiles.length-1?`1px solid ${SEP}`:"none",cursor:"pointer"}}>
              ...
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:15,fontWeight:700,color:ACC,letterSpacing:"-.5px"}}>{(s.xp||0).toLocaleString()}</div>
...
// ── COMMUNITY TAB ─────────────────────────────────────────────────────────
export function CommunityTab({profile,allProfiles,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG}){
  ...
  const loadMessages=async()=>{
    if(mode==="group"){
      ...
      const{data}=await supabase.from("messages").select("*").eq("is_dm",true).or(`and(user_id.eq.${profile.id},recipient_id.eq.${dmWith.id}),and(user_id.eq.${dmWith.id},recipient_id.eq.${profile.id})`).order("created_at",{ascending:true}).limit(100);
  ...
  const startDM=user=>{setDmWith(user);setMode("dm");setShowPeople(false);setDmMessages([]);};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 170px)"}}>
      ...
      <div style={{display:"flex",gap:8,padding:"0 16px 10px",borderBottom:`1px solid ${SEP}`,background:BG,flexShrink:0}}>
        <button onClick={()=>{setMode("group");setDmWith(null);}} style={{flex:1,padding:"8px",background:mode==="group"?ACC:"rgba(0,0,0,.06)",color:mode==="group"?"#fff":LB2,border:"none",borderRadius:9,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SF}}>💬 Group Chat</button>
        <button onClick={()=>setShowPeople(true)} style={{flex:1,padding:"8px",background:mode==="dm"?ACC:"rgba(0,0,0,.06)",color:mode==="dm"?"#fff":LB2,border:"none",borderRadius:9,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SF}}>
          {mode==="dm"&&dmWith?`💌 ${dmWith.nickname||dmWith.name?.split(" ")[0]}`:"💌 Direct Message"}
        </button>
      </div>

      {/* People picker for DM */}
      {showPeople&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:100,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
          <div style={{background:BG,borderRadius:"20px 20px 0 0",padding:"20px 16px 40px",maxHeight:"60vh",overflowY:"auto"}}>
            <div style={{fontSize:17,fontWeight:600,color:LBL,marginBottom:14,letterSpacing:"-.3px"}}>Select Person</div>
            {allProfiles.filter(p=>p.id!==profile.id).map(u=>(
      ...
      <div style={{padding:"8px 16px",background:BG2,borderTop:`1px solid ${SEP}`,display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
        <input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} placeholder={mode==="dm"?`Message ${dmWith?.nickname||dmWith?.name?.split(" ")[0]}…`:"Message your team…"}
        ...
        <button onClick={send} disabled={!text.trim()||sending} style={{width:40,height:40,borderRadius:"50%",background:text.trim()?ACC:"#e5e5ea",border:"none",cursor:text.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,color:"#fff",transition:"background .15s"}}>
          {sending?"…":"▶"}
        </button>
      </div>
    </div>
...
export function ProfileTab({profile,syncProfile,score,tier,completedCount,showToast,SF,BG,BG2,SEP,LBL,LB2,LB3,ACC,ORG}){
  const [section,setSection]=useState("public"); // public | private
  const [editing,setEditing]=useState(false);
  const [editPrivate,setEditPrivate]=useState(false);
  const [form,setForm]=useState({nickname:profile.nickname||"",bio:profile.bio||"",hobby:profile.hobby||"",favorite_food:profile.favorite_food||"",contact_number:profile.contact_number||"",avatar_url:profile.avatar_url||"",banner_url:profile.banner_url||""});
  ...
  useEffect(()=>{
    supabase.from("verification_requests").select("*").eq("user_id",profile.id).order("submitted_at",{ascending:false}).then(({data})=>{if(data)setVerif(data);});
  },[]);
  ...
  const submitBankChange=async()=>{
    ...
    if(!/^\d+$/.test(privateForm.bank_account)){showToast("Numbers only for bank account");return;}
    setSaving(true);
    ...
    <div style={{paddingBottom:12}}>
      ...
      <div style={{padding:"0 16px",marginTop:-44,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        <div style={{position:"relative"}}>
          ...
          {editing&&<button onClick={()=>document.getElementById("avProfile").click()} style={{position:"absolute",bottom:2,right:2,width:26,height:26,background:ACC,border:`2px solid ${BG}`,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:13}}>📷</button>}
      ...
      <div style={{display:"flex",gap:8,padding:"0 16px",marginBottom:14}}>
        {[["public","Public Info"],["private","My Private Info 🔒"]].map(([id,label])=>(
          <button key={id} onClick={()=>setSection(id)} style={{flex:1,padding:"9px",background:section===id?ACC:"rgba(0,0,0,.06)",color:section===id?"#fff":LB2,border:"none",borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SF}}>{label}</button>
        ))}
      </div>
        ...
        editing?(
          <div style={{padding:"0 16px"}}>
            <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:10}}>
              {[["Nickname","nickname","Your nickname"],["Hobby","hobby","e.g. Gaming"],["Favourite Food","favorite_food","e.g. Nasi Lemak"]].map(([label,key,ph],i,a)=>(
                <div key={key} style={{padding:"11px 16px",borderBottom:i<a.length-1?`1px solid ${SEP}`:"none"}}>
                  ...
                  <input value={form[key]} onChange={e=>set(key,e.target.value)} placeholder={ph} style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL,fontFamily:SF}}/>
                </div>
              ))}
              <div style={{padding:"11px 16px",borderBottom:`1px solid ${SEP}`}}>
                ...
                <textarea value={form.bio} onChange={e=>set("bio",e.target.value)} placeholder="Short description…" rows={3} style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL,resize:"none",fontFamily:SF,lineHeight:1.45}}/>
              </div>
              <div style={{padding:"11px 16px"}}>
                ...
                <input value={form.contact_number} onChange={e=>set("contact_number",e.target.value.replace(/\D/g,''))} placeholder="0123456789" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL,fontFamily:SF}}/>
          ...
          <div style={{padding:"0 16px"}}>
            <div style={{background:BG2,borderRadius:13,overflow:"hidden"}}>
              {[
                ...
                ["🍜 Favourite Food",profile.favorite_food||"Not set"],
                ["📱 Contact",       profile.contact_number?formatContact(profile.contact_number):"Not set"],
      ...
      {section==="private"&&(
        <div style={{padding:"0 16px"}}>
          ...
          {/* Bank account change (always allowed) */}
          {!editPrivate?(
            ...
            <div style={{background:BG2,borderRadius:13,overflow:"hidden",marginBottom:10}}>
              <div style={{padding:"11px 16px",borderBottom:`1px solid ${SEP}`}}>
                ...
                <input value={privateForm.bank_account} onChange={e=>setPrivateForm(p=>({...p,bank_account:e.target.value.replace(/\D/g,'')}))} placeholder="Numbers only" type="tel" style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:17,color:LBL,fontFamily:SF}}/>
              </div>
              <div style={{padding:"11px 16px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
                  {BANK_TYPES.slice(0,8).map(b=>(
                    <button key={b} onClick={()=>setPrivateForm(p=>({...p,bank_type:b}))} style={{padding:"7px 8px",background:privateForm.bank_type===b?ACC:"rgba(0,0,0,.05)",color:privateForm.bank_type===b?"#fff":LBL,border:"none",borderRadius:7,fontSize:12,cursor:"pointer",fontFamily:SF,textAlign:"left"}}>
                      {b}
                    </button>
                  ))}
                </div>
